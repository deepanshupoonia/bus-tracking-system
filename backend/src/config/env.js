...
export const env = {
  ...
  corsOriginPatterns: parseCorsOrigins(process.env.CORS_ORIGIN_PATTERN ?? ''),
...
export function isCorsOriginAllowed(origin) {
  ...
  return env.corsOrigins.includes(normalized) || env.corsOriginPatterns.some((pattern) => matchesOriginPattern(normalized, pattern));
}
