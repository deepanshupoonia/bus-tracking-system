...
export async function list(request,response,next) { try { response.json({success:true,data:{announcements:await announcements.listAnnouncements(request.user.sub)}}); } catch (error) { next(normalizedError(error)); } }
...
export async function markRead(request,response,next) { try { response.json({success:true,data:{read:await announcements.markAnnouncementRead(request.user.sub,Number(request.params.id))}}); } catch (error) { next(normalizedError(error)); } }
