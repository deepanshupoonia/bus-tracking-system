...
export function DriverDashboard() {
  ...
  const stopGps = () => { navigator.geolocation?.clearWatch(watch.current); clearInterval(timer.current); };
  ...
  const beginGps = () => {
    ...
    watch.current = navigator.geolocation.watchPosition((position) => { latestPosition.current = position; }, () => setError('GPS permission was denied or location is unavailable.'), { enableHighAccuracy: true, maximumAge: 5000, timeout: 10000 });
    clearInterval(timer.current); timer.current = setInterval(sendLocation, GPS_UPDATE_INTERVAL_MS);
  };
  const start = async () => { await api.post('/driver/start-route'); beginGps(); load(); };
